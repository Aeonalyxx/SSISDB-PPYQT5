...
class EditStudentDialog(QtWidgets.QDialog):
    ...
    def save_and_close(self):
        if not all((field.text() if isinstance(field, QtWidgets.QLineEdit) else field.currentText()) 
               for field in [self.I_edit, self.Name_edit, self.Gender_combobox, self.Year_combobox]):
            QtWidgets.QMessageBox.critical(self, "Error", "Please fill in all fields.")
        else:
            edited_student_data = [
                self.I_edit.text(),
            ...
            if edited_student_data[0] != self.original_student_data[0]:
                if not self.check_ID_unique(edited_student_data[0]):
                    QtWidgets.QMessageBox.warning(self, "Warning", "Student ID already exists. Please enter a unique ID.")
                    return
    ...
    def check_ID_unique(self, new_ID):
        conn = sqlite3.connect('ssis.db')
        cursor = conn.cursor()
        cursor.execute('SELECT * FROM studentlist WHERE Student_ID = ?', (new_ID,))
        result = cursor.fetchone()
...
#STUDENT FUNCTIONS=========================================================================================================
    ...
    def search_studentlist(self):
        ...
        search_query = self.StudentIDLineEdit.text().strip().upper()  
        ...
        for row_data in cursor.fetchall():
            if filter_option == "Student_ID" and search_query in row_data[0].lower():
                self.add_row_to_student_table(row_data)
                matches_found = True
            elif filter_option == "Name" and search_query in row_data[1].upper():
                self.add_row_to_student_table(row_data)
                matches_found = True
            elif filter_option == "Gender" and search_query == row_data[2].upper():
                self.add_row_to_student_table(row_data)
                matches_found = True
            elif filter_option == "Course_Code" and search_query in row_data[3].upper():
                    self.add_row_to_student_table(row_data)
                    matches_found = True
        ...
        if selected_row >= 0:
            ...
            if confirmation == QtWidgets.QMessageBox.Yes:
                ...
                if course_code_item is not None:
                    ...
                    try:
                        cursor.execute("DELETE FROM courselist WHERE Course_Code = ?", (course_code,))
                        cursor.execute("UPDATE studentlist SET Course_Code = NULL WHERE Course_Code = ?", (course_code,))
                        cursor.execute("UPDATE studentlist SET Course_Code = 'Not Enrolled' WHERE Course_Code IS NULL")
                        conn.commit()
                        ...
                        QtWidgets.QMessageBox.critical(self.centralwidget, "Error", f"Database error: {str(e)}")
                    finally:
                        conn.close()
